...
--Using a while loop and a temporary table in a stored procedure
CREATE PROC
	usp_backupDatabases
AS
BEGIN
...
--try deleting from the temporary table after the backup instead of having a column for row number
CREATE PROC
	usp_backupDatabasesAlt
AS
BEGIN
	SET NOCOUNT ON
	...
	BEGIN
		...
		--delete the database name from the temporary table
		DELETE FROM 
			#temp_info
		WHERE
			DatabaseName = @databaseName
